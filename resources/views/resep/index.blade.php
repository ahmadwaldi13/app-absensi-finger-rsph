...
@endsection
@else
@section('title-header', 'Ubah Resep')
@endif
...
@section('content')
<div>
    @include('layouts.tab_resep',['active'=>1])
                        ...
                        @if(!empty($data_list))
                            @foreach($data_list as $item)
                                <?php
                                    ...
                                    if( (new \App\Http\Traits\AuthFunction)->checkAkses('resep/fullAkses') ){
                                        $check_akses=1;
                                    }
                                ?>
                                <tr>
                                    ...
                                    <td>
                                        @if( (new \App\Http\Traits\AuthFunction)->checkAkses('resep/view') )
                                            <a href="{{ url('resep/view') }}" class='modal-remote' data-modal-key='{{ $kode }}' data-modal-title='View Isi Resep'>
                                                <img src="{{ asset('') }}icon/info.png" alt="">
                                            </a>
                                        @endif
                                        @if($check_akses)
                                            @if( (new \App\Http\Traits\AuthFunction)->checkAkses('resep/update') )
                                                <a href="{{ url('resep/form_update?data_sent='.$kode) }}">
                                                    <img src="{{ asset('') }}icon/edit.png" alt="">
                                                </a>
                                            @endif
                                            
                                            @if( (new \App\Http\Traits\AuthFunction)->checkAkses('resep/delete') )
                                                <a href="{{ url('resep/delete') }}" class='modal-remote-delete' data-modal-key='{{ $kode }}' data-confirm-message="Apakah anda yakin menghapus data ini ?">
