                ...
                @if(!empty($list_data))
                    @foreach($list_data as $key => $item)
                        <?php
                            ...
                            if( (new \App\Http\Traits\AuthFunction)->checkAkses('pemeriksaan-radiologi/fullAkses') ){
                                $check_akses=1;
                            }
                        ?>
                        ...
                        <tr>
                            ...
                            <td>{{ $waktu_hasil  }}</td>
                            <td>
                                <a href='pemeriksaan-radiologi/view' class='btn btn-kecil btn-info modal-remote' data-modal-key='{{ $kode }}' data-modal-width='90%' data-modal-title='View Pemeriksaan Radiologi'><i style='color:#fff;' class="fa-solid fa-circle-info"></i></a>
                                @if($check_akses)
                                    <a href='pemeriksaan-radiologi/delete' class='btn btn-kecil btn-danger modal-remote-delete' data-modal-key='{{ $kode }}' data-confirm-message="Apakah anda yakin menghapus data ini ?"><i class="fa-solid fa-trash"></i></a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
