    ...
    <div id="tindakan-pasien">
        <ul class="nav nav-tabs">
            <li class="nav-item border-radius-top text-center" style="width: 300px; background-color: #F5F5F5;">
                <a class="nav-link border-radius-top tabs text-muted {{ ($tabs==null) ? 'active' : '' }}" id="tabs-daftar" aria-current="page" href="{{ $tabLink }}">Daftar Tindakan / Tagihan</a>
            </li>
            <li class="nav-item border-radius-top text-center mx-2" style="width: 300px; background-color: #F5F5F5;">
                <a class="nav-link border-radius-top tabs text-muted {{ $tabs=='tindakan' ? 'active' : '' }}" id="tabs-tindakan" href="{{ $tabLink }}&tab=tindakan">Tindakan Dilakukan @if($jmlTindakan > 0)<span>({{ $jmlTindakan }})</span>@endif</a>
            </li>
        </ul>
