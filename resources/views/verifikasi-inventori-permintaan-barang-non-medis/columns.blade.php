                    ...
                    <div class="col-lg-3 col-md-10">
                        <div class='bagan_form'>
                            ...
                            <div class="button-icon-inside">
                                ...
                                <span class="modal-remote-data" data-modal-src="{{ url('ajax?action=get_list_departemen') }}" data-modal-key="" data-modal-pencarian='true' data-modal-title='Departemen' data-modal-width='40%' data-modal-action-change="function=.set-data-list-from-modal@data-target=#filter_nm_departemen|#filter_kd_departemen@data-key-bagan=0@data-btn-close=#closeModalData">
                    ...
                    @if(!empty($get_status_verifikasi->status_verifikasi))
                        <div class="col-lg-2 col-md-10">
                            ...
                            <select class='form-control form-select' id='form_filter_status' name="form_filter_status">
                                <option value="all" {{ ( $form_filter_status == 'all') ? 'selected' : '' }} >Semua</option>
                                @foreach ($get_status_verifikasi->status_verifikasi as $key => $value)
                                    <option value="{{ $key }}" {{ ( $form_filter_status == (string)$key) ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @foreach($list_data as $key => $item)
                                <?php
                                    ...
                                    if( (new \App\Http\Traits\AuthFunction)->checkAkses($router_name->uri.'/fullAkses') ){
                                        if($check_status->status==2 or $check_status->status==3){
                                            $check_status2=(new \App\Models\UxuiPermintaanBarangNonMedisStatus)->get_status_verifikasi($item->status_veri,$item->status,$item->no_permintaan);
                                            if(count($check_status2->list_verifikasi)>=2){
                                                $update_verifikasi=1;
                                            }
                                        }
                                    }
                                ...
                                <tr>
                                    ...
                                    <td class='text-right'>
                                        ...
                                        @elseif( (new \App\Http\Traits\AuthFunction)->checkAkses($router_name->uri.'/fullAkses') )
                                            @if( $update_verifikasi==1 )
                                                {!!  (new \App\Http\Traits\AuthFunction)->setPermissionButton([$router_name->uri.'/verifikasi',$paramater_url,'update','','Ubah Verifikasi'],'') !!}
                                            @endif
                                        @endif
                                        @if (!empty($get_status_verifikasi->status_verifikasi))
