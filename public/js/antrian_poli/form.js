...
async function panggilPasien(namaPasien, noreg, from, patientTableRow) {
    const queryString = window.location.search;
    const voiceBahasa = getBahasa();
    const msg = new SpeechSynthesisUtterance();
    // msg.text = `${namaPasien}. nomor antrian ${loket} silahkan menuju ke ${from}`.toUpperCase();
    // msg.text = `${namaPasien}. nomor antrian ${noreg} silahkan menuju ke ${from}`;
    msg.text = `${namaPasien}. silahkan menuju ke ${from}`;
...
async function getPasien(p_id) {
    const el = document.getElementById(p_id);
    const data = el.value;
    const [no_reg, nama, from] = data.split(",");
    const patientTableRow = el.parentElement.parentElement

    await panggilPasien(nama, no_reg, from, patientTableRow)
...
$("#modalListMonitor #submitListMonitor").click(function () {
    post(`${base_url}/antrian-poliklinik-petugas/masuk-monitor`, patientData)
})
