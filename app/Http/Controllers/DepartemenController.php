...
class DepartemenController extends \App\Http\Controllers\MyAuthController
{
    public function __construct()
    {
        ...
        $this->title = 'Referensi Departemen / Bidang';
        $this->breadcrumbs = [
    ...
    private function proses($request)
    {
        ...
        try {
            $model = (new \App\Models\RefDepartemen)->where('id_departemen', '=', $kode)->first();
            if (empty($model)) {
                $model = (new \App\Models\RefDepartemen);
            }
            $data_save = $req;
