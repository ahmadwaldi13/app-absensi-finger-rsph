...
class JadwalAbsensiController extends \App\Http\Controllers\MyAuthController
{
    ...
    private function proses($request)
    {
        ...
        try {
            ...
            $data_save = $req;
            $data_save['uraian'] = $data_save['uraian'];
            $data_save['jam_awal'] = $data_save['jam_awal'];
            $data_save['jam_akhir'] = $data_save['jam_akhir'];
            $data_save['alias'] = strtolower(str_replace(" ", "_", $this->globalFunction->remove_multiplespace($data_save['alias'])));
