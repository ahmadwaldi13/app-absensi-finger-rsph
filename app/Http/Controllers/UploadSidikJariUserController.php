...
class UploadSidikJariUserController extends \App\Http\Controllers\MyAuthController
{
    ...
    public function __construct()
    {
        ...
        $this->title = 'Upload Sidik Jari User';
        $this->breadcrumbs = [
    ...
    function getListSidikJariMasterForm(Request $request)
    {
        $hasil_data = [];
        try {
            $form_filter_text = '';
    ...
    private function form(Request $request)
    {
        ...
        if ($model) {
            ...
            $list_item = $this->refKaryawanService->getList_finger($kode);
            $item_list_terpilih = [];
            if ($list_item) {
        ...
        return view('upload-sidik-jari-user.form', $parameter_view);
    }
    ...
    private function proses($request){
        ...
        $mesin=(new \App\Services\MesinFinger($data_mesin->ip_address));
        $get_user=$mesin->get_user_upload_sidik_jari('',$item_list_terpilih);
    ...
    function ajax(Request $request)
    {
        ...
        if ($get_req['action'] == 'list_sidik_jari_master_form') {
            $hasil = $this->getListSidikJariMasterForm($request);
            if ($request->ajax()) {
                $return = '';
                if ($hasil) {
