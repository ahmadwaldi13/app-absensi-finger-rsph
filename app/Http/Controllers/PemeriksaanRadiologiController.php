...
class PemeriksaanRadiologiController extends Controller
{
    ...
    private function data_form($params){
        ...
        if ($no_rawat && $no_rm && $type_form) {
            $no_order=$this->pemeriksaanRadiologiService->getNoOrder(date('Y-m-d'));
            ...
            $pemeriksaan_list1=$this->pemeriksaanRadiologiService->getjnsPerawatanRadiologi(['status'=>'1']);
    ...
    function actionIndex(Request $request){
        ...
        if ($item_pasien->no_rawat && $item_pasien->no_fr) {
            ...
            if($item_pasien->no_fr=='ri'){
                $list_data=$this->pemeriksaanRadiologiService->getListRanap(['permintaan_radiologi.no_rawat'=>$noRawat,'permintaan_radiologi.status'=>'ranap','tgl_permintaan'=>[$filter_start,$filter_end]]);
            }else{
                $list_data=$this->pemeriksaanRadiologiService->getListRalan(['permintaan_radiologi.no_rawat'=>$noRawat,'permintaan_radiologi.status'=>'ralan','tgl_permintaan'=>[$filter_start,$filter_end]]);
            }
    ...
    {
        DB::beginTransaction();
        $link_back='pemeriksaan_radiologi';
        $item_pasien=(new \App\Http\Traits\ItemPasienFunction)->getItemPasien($request->fr);
        ...
        try {   
            ...
            $rules = [
                'noorder' => 'required|unique:permintaan_radiologi'
            ];
            ...
            $no_order=$this->pemeriksaanRadiologiService->getNoOrder($data_save['tgl_permintaan']);
            ...
            $no_order=$this->pemeriksaanRadiologiService->getNoOrder($data_save['tgl_permintaan']);
            $data_save=$this->pemeriksaanRadiologiService->permintaanRadiologi->set_columns_table_with_data($data_save);
            $data_save['noorder']=$no_order;
        
            $model_permintaan=$this->pemeriksaanRadiologiService->insert($data_save);
            if($model_permintaan){
                foreach($data_periksa as $key => $value){
                    ...
                    $model_permintaan_lab=$this->pemeriksaanRadiologiService->insert_pemeriksaan_radiologi($paramater);
    ...
    function actionDelete(Request $request)
    {
        ...
        try {  
            if($type_akses=='ri'){
                $model=$this->pemeriksaanRadiologiService->getListRanap(['permintaan_radiologi.noorder'=>$noorder,'permintaan_radiologi.status'=>'ranap']);
            }else{
                $model=$this->pemeriksaanRadiologiService->getListRalan(['permintaan_radiologi.noorder'=>$noorder,'permintaan_radiologi.status'=>'ralan']);
            }
            ...
            if(!empty($model)){
                ...
                if(!empty($noorder)){
                    $delete_model=$this->pemeriksaanRadiologiService->delete(['noorder'=>$noorder,'stts_bayar'=>'Sudah']);
    ...
    function actionView(Request $request)
    {
        ...
        $list_pp_lab=[];
        if(!empty($model)){
            $data=$this->pemeriksaanRadiologiService->getPermintaanPemeriksaanRadiologi(['permintaan_pemeriksaan_radiologi.noorder'=>$model->noorder]);
            if(!empty($data)){
        ...
        if($request->ajax()){
            $returnHTML = view('pemeriksaan-radiologi.view',$parameter_view)->render();
    ...
    function getNoOrder(Request $request){
        ...
        return $this->sendSuccess($this->pemeriksaanRadiologiService->getNoOrder($request->tgl), "Success");
    }
}