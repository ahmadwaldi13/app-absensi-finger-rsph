...
class ResepController extends Controller
{
    ...
    private function data_form($params){
        ...
        if ($no_rawat && $no_rm && $type_form) {
            ...
            $list_barang=$this->resepService->getListBarang(['gudangbarang.kd_bangsal'=>$bangsal]);
            ...
            $no_resep=$this->resepService->getNoResep(date('Y-m-d'));
    ...
    private function getCopyResep($params){
        ...
        $get_data=$this->resepService->getResepListDetail($paramater_search);
        $get_data=!empty($get_data[0]) ? $get_data[0] : (object)[];

        $list_obat_terpilih=[];
        if(!empty($get_data->item_obat)){
            if($type_akses=='ri'){
                $bangsal=$this->resepService->getKamar(['kamar_inap.no_rawat'=>$no_rawat]);
                $bangsal=!empty($bangsal[0]) ? $bangsal[0]->kd_bangsal : '';
            ...
            foreach($get_data->item_obat as $value){
                $list_barang=$this->resepService->getListBarang(['gudangbarang.kd_bangsal'=>$bangsal,'databarang.kode_brng'=>$value->kode_brng]);
                $list_barang=!empty($list_barang[0]) ? (object)$list_barang[0]->getAttributes() : [];

                $harga=!empty($list_barang->h_beli) ? $list_barang->h_beli : 0;
                $list_obat_terpilih[]=json_encode([
                    "kode_barang"=>!empty($value->kode_brng) ? $value->kode_brng : 0,
                    "nm_barang"=>!empty($value->nama_brng) ? $value->nama_brng : 0,
                    "satuan"=>!empty($value->kode_sat) ? $value->kode_sat : 0,
                    'harga'=>(new \App\Http\Traits\GlobalFunction)->formatMoney($harga),
                    'harga_real'=>$harga,
                    'stok'=>!empty($list_barang->stok) ? $list_barang->stok : 0,
                    'kapasitas'=>!empty($list_barang->kapasitas_brng) ? $list_barang->kapasitas_brng : 0,
                    'jlh_input'=>!empty($value->jml) ? $value->jml : 0,
                    'aturan_pakai'=>!empty($value->aturan_pakai) ? $value->aturan_pakai : '',
                    'checked_box'=>0,
                    'id_tr'=>!empty($value->kode_brng) ? $value->kode_brng : 0,
                ]);
            }
        }

        return $list_obat_terpilih;
    }
    ...
    function actionIndex(Request $request){
        ...
        if ($item_pasien->no_rm && $item_pasien->no_rawat && $item_pasien->no_fr) {
            ...
            $data_list=$this->resepService->getResepList($paramater_search);

            $parameter_view=[
                'action_form'=>'resep/create',
                'data_list'=>$data_list,
                'data_feedback_item'=>!empty($data_feedback_item) ? $data_feedback_item : null
            ];

            $parameter_view=array_merge($parameter_view,$parameter_data_form);

            return view('resep.index',$parameter_view);
        }
    ...
    function actionCreate(Request $request)
    {
        ...
        $data_feedback=!empty($data_request['list_obat']) ? $data_request['list_obat'] : [];
        ...
        try {
            ...
            $data_obat=$model_resep_obat["list_obat"];
            unset($model_resep_obat["list_obat"]);

            $no_resep='';
            if($model_resep_obat['tgl_peresepan']){
                $no_resep=$this->resepService->getNoResep($model_resep_obat['tgl_peresepan']);
            }
            ...
            foreach ($data_obat as $key => $value) {
                ...
                $jml=$data->jlh_input;
                if($data->checked_box){
                    $kapasitas=$this->resepService->getKapasitasBarang(['databarang.kode_brng'=>$data->kode_barang]);
                    $kapasitas=!empty($kapasitas[0]) ? $kapasitas[0] : [];
                    $kapasitas=!empty($kapasitas->kapasitas) ? $kapasitas->kapasitas : 0;
                    if($kapasitas!=0){
                        $jml=$data->jlh_input/$kapasitas;
                    }
                }

                $model_resep_dokter[]=[
                    'no_resep'=>$no_resep,
                    'kode_brng'=>$data->kode_barang,
                    'jml'=>$jml,
            ...
            $data_save=0;
            $model_save=$this->resepService->resepObat->insert($model_resep_obat);
            if($model_save){
                foreach ($model_resep_dokter as $item) {
                    $item['no_resep']=$no_resep;
                    $model_aturan_pakai=$this->resepService->checkAturanPakai($item['aturan_pakai']);
                    $model_save_2=$this->resepService->resepDokter->insert($item);
                    if($model_save_2){
                        $data_save++;
                    }else{
                        $data_save--;
            ...
            $is_save=0;
            if(count($model_resep_dokter)==$data_save){
    ...
    function formUpdate(Request $request)
    {
        ...
        if ($no_rm && $no_rawat && $no_resep && $type_akses) {
            ...
            $check=$this->resepService->getResepList($paramater_search);
            ...
            if(!empty($model->item_obat)){
                ...
                foreach($model->item_obat as $value){
                    $list_barang=$this->resepService->getListBarang(['gudangbarang.kd_bangsal'=>$bangsal,'databarang.kode_brng'=>$value->kode_brng]);
                    $list_barang=!empty($list_barang[0]) ? (object)$list_barang[0]->getAttributes() : [];
    ...
    function actionUpdate(Request $request)
    {
        ...
        $link_back_error='resep_form_update';
        ...
        try {
            ...
            $model_resep_obat_old=$this->resepService->getResepList($paramater_search);
            ...
            $model_update=$this->resepService->update($paramater_where,$model_resep_obat);
            if($model_update){
                $data_save++;
            }else{
                $data_save--;
            ...
            $model_resep_delete=$this->resepService->delete_resep_dokter($paramater_where);
            if($model_resep_delete){
                foreach ($model_resep_dokter as $item) {
                    $item['no_resep']=$model_resep_obat['no_resep'];
                    $model_aturan_pakai=$this->resepService->checkAturanPakai($item['aturan_pakai']);
                    $model_save_2=$this->resepService->resepDokter->insert($item);
                    if($model_save_2){
                        $data_save++;
            ...
            DB::rollBack();
            if($e->errorInfo[1] == '1062'){
                return redirect()->route($link_back_error, $link_back_param_error)->with(['error' => 'Maaf tidak dapat menyimpan data yang sama','data_feedback'=>$data_feedback]);
            }
    ...
    function actionDelete(Request $request)
    {
        ...
        try {
            ...
            if(!empty($no_resep)){
                ...
                $delete_model=$this->resepService->delete(['no_resep'=>$no_resep]);
                if ($delete_model) {
                    DB::commit();
    ...
    function actionView(Request $request)
    {
        ...
        $model=$this->resepService->getResepListDetail($paramater_search);
        ...
        if($request->ajax()){
            $returnHTML = view('resep.view',$parameter_view)->render();
            return response()->json(array('success' => true, 'html'=>$returnHTML));
        }else{
            return view('resep.view',$parameter_view);
        }
    }

    function getAturanPakai(Request $request)
    {
        if($request->search){
            return $this->resepService->aturanPakai->where('aturan', 'LIKE', '%' . $request->search . '%')->get();
        }
    }
}