...
class BarangNonMedisController extends \App\Http\Controllers\MyAuthController
{
    public function __construct() {
        ...
        $this->barangNonMedisService = new BarangNonMedisService;        
    }
    ...
    function actionIndex(Request $request){
        ...
        $list_data=$this->barangNonMedisService->getBarangList($paramater,1)->paginate(!empty($request->per_page) ? $request->per_page : 15);
    ...
    private function form(Request $request){   
        ...
        $paramater=[
            'ipsrsbarang.status'=>['=','1'],
            'ipsrsbarang.kode_brng'=>$kode,
        ];

        $model=$this->barangNonMedisService->getBarangList($paramater,1)->first();
        if ($model) {
    ...
    private function proses($request){
        ...
        try {
            $model = (new \App\Models\IpsrsBarang)->where('kode_brng', '=', $kode)->first();
            if(empty($model)){
                $model=(new \App\Models\IpsrsBarang);
            }
            $data_save=$req;
            $data_save['status']=1;
