...
use App\Services\KodeBarangNonMedisPenunjangService;

class KodeBarangNonMedisPenunjangController extends \App\Http\Controllers\MyAuthController
{
    public function __construct()
    {
        ...
        $this->title = 'Kode Barang Non Medis';
        $this->breadcrumbs = [
        ...
        $this->kodeBarangNonMedisPenunjangService = new KodeBarangNonMedisPenunjangService;
    }
    ...
    function actionIndex(Request $request)
    {
        ...
        $list_data = $this->kodeBarangNonMedisPenunjangService->getList($paramater, 1)->paginate(!empty($request->per_page) ? $request->per_page : 15);
    ...
    private function form(Request $request)
    {
        ...
        $paramater = [
            'ipsrsjenisbarang.kd_jenis' => $kode
        ];
        $model = $this->kodeBarangNonMedisPenunjangService->getList($paramater, 1)->first();
        if ($model) {
