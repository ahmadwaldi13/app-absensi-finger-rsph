    ...
    public function getUserByCredential(string $id_user, string $password){
        try{
            if(!empty($id_user) &&  !empty($password)){
                ...
                $model = (new \App\Models\UserManagement\UxuiUsers)->select(['id','username',DB::raw("AES_DECRYPT(password,'jiwana') password")])->where('username', '=', $id_user)->where('status', '=', 1)->first();
                if(empty($model)){
